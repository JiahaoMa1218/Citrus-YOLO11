...
We have detailed the LLKFE module code in ultralytics/ultralytics/nn/modules/block.py. This module implementation focuses on enlarging the receptive field to effectively handle targets under severe occlusion.

AFR (Adaptive Feature Refinement)
We have detailed the AFR module code in ultralytics/ultralytics/nn/modules/block.py. This module is specifically designed to filter out complex orchard backgrounds, such as leaves and branches, significantly reducing false positives.

DPSPP (Dual-Path Spatial Pyramid Pooling)
We have detailed the DPSPP module code in ultralytics/ultralytics/nn/modules/block.py. This structure is proposed to enhance the network's multi-scale feature extraction capabilities, ensuring robust detection across different fruit sizes.

CAMA (Content-Aware Multiscale Aggregation)
We have detailed the CAMA mechanism code in ultralytics/ultralytics/nn/modules/block.py. This module is crucial for the neck to achieve efficient feature fusion and upsampling, improving both accuracy and inference speed.
...
Test (FPS Evaluation)
The ultralytics/test.py script is used for performing inference and FPS evaluation on images using the trained model to verify real-time performance:
python ultralytics/test.py
